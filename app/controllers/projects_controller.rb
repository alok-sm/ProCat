...
@every=@project.guides.to_s.split(";")
...
oldg.each { |oldg_each|
	if (newg.include?(oldg_each))
	else
		puts "im in delete guide"
     if User.where(:code => oldg_each).present?
		@users = User.where(:code => oldg_each)
		c=@users.first
		...
		@shadow = Shadow.where(:code => oldg_each)
		c=@shadow.first
